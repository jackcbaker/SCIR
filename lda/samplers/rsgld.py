...
#            print sampler_name + " iteration %d, docs so far %d, log pred %g, time %g" % (t, docs_so_far, ho_lp, end - start)
            # Assess holdout log-pred every 500 documents
            if t % 5 == 0:
                ho_lp = self.holdout_log_pred(holdout_train_cts, holdout_test_cts, samples_per_update)
                print "Iteration: {0}\tlog-pred: {1}\tn_docs: {2}".format(t, np.exp(-ho_lp), docs_so_far)
                self.write_log(t, np.exp(-ho_lp), docs_so_far)
                self.ho_log_preds.append([docs_so_far, ho_lp])
            t += 1
    ...
    def write_log(self, t, ho_lp, docs_so_far):
        with open('cluster/lda-rsgld/out-{0}'.format(self.seed), 'a') as outfile:
