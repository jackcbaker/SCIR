    ...
    for seed in xrange(5, 10):
        ...
        for n in [1, 10, 50, 100, 500]:
            for h in [1.0, 5e-1, 1e-1, 5e-2, 1e-2, 5e-3, 1e-3]:
                ...
                np.save("data/dense/cir-score-{0}-{1}-{2}".format(h, n, seed), sampler.score)
                np.save("data/dense/cir-times-{0}-{1}-{2}".format(h, n, seed), sampler.times)
            for h in [1e-3, 5e-4, 1e-4, 5e-5, 1e-5, 5e-6, 1e-6]:
                print "RSGLD\t{0}\t{1}\t{2}".format(n, h, seed)
                np.random.seed(seed)
                sampler = RSGLD(model, h, n, verbose = False, M = 10 ** 3) 
                print sampler.score[-1]
                np.save("data/dense/rsgld-score-{0}-{1}-{2}".format(h, n, seed), sampler.score)
                np.save("data/dense/rsgld-times-{0}-{1}-{2}".format(h, n, seed), sampler.times)
        np.random.seed(seed)
        print "Direct\t{0}".format(seed)
        sampler = Direct(model, verbose = False, M = 10 ** 3)
        print sampler.score[-1]
        np.save("data/dense/direct-score-{0}".format(seed), sampler.score)
...
def plot_n():
    ...
    for n in [1, 10, 50, 100, 500]:
        ...
        for h in [1.0, 5e-1, 1e-1, 5e-2, 1e-2, 5e-3]:
    ...
    plot_frame.to_csv('data/dense_n_processed.csv', index = False)
        ...
        for h in [1.0, 5e-1, 1e-1, 5e-2, 1e-2, 5e-3, 1e-3]:
            ...
            for i, seed in enumerate(xrange(5, 10)):
                scores['CIR'][n][h][:, i] = np.load(
                        "data/dense/cir-score-{0}-{1}-{2}.npy".format(h, n, seed))
                times['CIR'][n][h][:, i] = np.load(
                        "data/dense/cir-times-{0}-{1}-{2}.npy".format(h, n, seed))
        for h in [1e-3, 5e-4, 1e-4, 5e-5, 1e-5, 5e-6, 1e-6]:
            scores['RSGLD'][n][h] = np.zeros((10 ** 3, 5))
            times['RSGLD'][n][h] = np.zeros((10 ** 3, 5))
            for i, seed in enumerate(xrange(5, 10)):
                scores['RSGLD'][n][h][:, i] = np.load(
                        "data/dense/rsgld-score-{0}-{1}-{2}.npy".format(h, n, seed))
                times['RSGLD'][n][h][:, i] = np.load(
                        "data/dense/rsgld-times-{0}-{1}-{2}.npy".format(h, n, seed))
        scores['Direct'][n] = np.zeros((10 ** 3, 5))
        for i, seed in enumerate(xrange(5, 10)):
            scores['Direct'][n][:, i] = np.load(
                    "data/dense/direct-score-{0}.npy".format(seed))
    return scores, times
